...
export const journalEntryCreateInputZod = journalEntryMandatoryZod.omit({ authorUserId: true }).extend(journalEntryOptionalZod.partial().shape).omit({ deletedAt: true });
export type JournalEntryCreateInput = z.infer<typeof journalEntryCreateInputZod>;

export const journalEntryUpdateInputZod = journalEntryMandatoryZod.extend(journalEntryOptionalZod.shape).extend(zTimestamps).partial().omit({ deletedAt: true });
