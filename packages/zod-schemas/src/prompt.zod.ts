...
export const promptCreateInputZod = promptMandatoryZod.extend(promptOptionalZod.partial().shape).omit({ deletedAt: true });
export type PromptCreateInput = z.infer<typeof promptCreateInputZod>;

export const promptUpdateInputZod = promptMandatoryZod.extend(promptOptionalZod.shape).extend(zTimestamps).partial().omit({ deletedAt: true });
